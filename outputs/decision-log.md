...
### D22. Provider calls stay server-side; progressive results use the control stream
Upload the selected 320×320 JPEG preview once to Hono over HTTP, call Groq and Exa directly from the server, and keep both provider keys out of the Expo bundle. The HTTP response remains the fallback identity result; the shared oRPC WebSocket publishes `identity.candidate` immediately after Groq and `evidence.patch` after a bounded Exa `instant` search without making the Groq response wait for Exa. Do not add AI SDK, AI Gateway, provider-facing WebSockets, structured Exa synthesis, or a streaming image relay before traces show a real need. The 2026-08-20 local live check used an 18,803-byte preview and measured Groq at 392 ms, Exa at 126 ms, and the complete identity-plus-evidence path at 526 ms; this is one Mac/network sample, not physical-device p50/p95 proof. Qwen 3.6 JSON mode must use `reasoning_effort: none`; leaving reasoning enabled produced a provider-side JSON validation 400 in the live check.
...
### D18. Drizzle is the B0 query layer, not a second migration authority
Use Drizzle ORM with Postgres.js immediately inside `apps/api` for typed queries and transactions. Define the prototype-owned tables in a dedicated `snap_to_post` PostgreSQL schema so they do not collide with the established marketplace's `public.items` lifecycle. Drizzle's TypeScript schema may generate reviewed SQL into `supabase/migrations`, but Supabase SQL migrations remain the only applied migration history. Do not run `drizzle-kit push` or maintain Drizzle's separate migration ledger against shared or remote databases. Hand-written migration SQL owns pgvector enablement, RLS/grants, Storage buckets/policies, and database functions outside Drizzle's model.

### D19. B0 writes require an isolated Supabase target
The Supabase MCP connection inspected on 2026-08-20 points at the established marketplace database: 113 public tables, 3,681 `public.items` rows, 8,089 objects in the existing `items` bucket, no `snap_to_post` schema, and no installed pgvector extension. The inspection was read-only. B0 must initialize and verify its schema against local Supabase or a fresh dev project; the connected marketplace project is reference-only and must never receive prototype tables, buckets, or writes.

### D20. Storage stays behind the backend boundary
Drizzle does not replace Supabase Storage. Create a private prototype bucket through reviewed Supabase migration/policy SQL, keep database and service credentials server-only, and have the Hono gateway issue narrowly scoped signed upload requests. The mobile app receives only the signed upload material required for the selected object path; it never receives a database URL or service-role/secret key.
