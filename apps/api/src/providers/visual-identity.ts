...
const DEFAULT_IDENTITY_MODEL = 'google/gemini-3.7-flash';
...
export async function identifyVisualItem(
  ...
  try {
    const result = await generateText({
      ...
      output: Output.object({ schema: VisualIdentityInferenceSchema }),
      messages: [
        {
          role: 'user',
          content: [
    ...
    const parsedInference = VisualIdentityInferenceSchema.safeParse(result.output);
    ...
    return {
      ...
      latencyMs,
      model: result.response.modelId || configuredModel,
